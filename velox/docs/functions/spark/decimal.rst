...
.. spark:function:: decimal_round(decimal[, scale]) -> [decimal]
    ...
    When ``abs(scale)`` exceeds the maximum precision of long decimal (38), the round logic is equivalent to the case where it is 38 as we cannot exceed the maximum precision. 
