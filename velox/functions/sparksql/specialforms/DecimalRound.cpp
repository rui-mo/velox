 ...
 private:
  inline TResult applyRound(const TInput input) const {
    if (scale_ >= 0) {
      ...
      VELOX_USER_CHECK(
          divideFactor_.has_value(),
          "Divide factor is needed for rounding to a negative scale.");
      VELOX_USER_CHECK(
          multiplyFactor_.has_value(),
          "Multiply factor is needed for rounding to a negative scale.");
