...
class DecimalAverageAggregate
    : public DecimalAggregate<TResultType, TInputType> {
 public:
  explicit DecimalAverageAggregate(TypePtr resultType, TypePtr sumType)
      : DecimalAggregate<TResultType, TInputType>(resultType),
        sumType_(sumType) {}
  ...
  TResultType computeFinalValue(
      LongDecimalWithOverflowState* accumulator) override {
    VELOX_UNREACHABLE();
  }
          ...
          switch (inputType->kind()) {
            ...
            case TypeKind::BIGINT: {
              if (inputType->isShortDecimal()) {
                ...
                VELOX_USER_CHECK_GE(inputPrecision, 12);
